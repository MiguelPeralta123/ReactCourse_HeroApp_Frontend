...
export const HeroGridCard = ({ hero }: Props) => {
    ...
    const { toggleFavorite, isFavorite } = use(FavoritesContext);

    return (
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
            <div className="relative h-64">
                <img
                    src={`${apiUrl}/images/${hero.image}`}
                    alt={hero.alias}
                ...
                <div className="absolute top-3 left-3 flex items-center gap-2">
                    <div className={ cn(hero.status == 'Active' ? 'bg-green-500' : 'bg-red-500', "w-3 h-3 rounded-full") } />
            ...
            <CardHeader className="py-3 z-10 bg-gray-100/50 backdrop-blur-sm relative top-1 group-hover:top-[24px] transition-all duration-300">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1">
                        {
                            hero && hero.powers.slice(0, 2).map(power =>
                                <Badge key={power} variant="outline" className="text-xs">{ power }</Badge>
                            )
                        }
                        {
                            hero && hero.powers.length > 2 ?
